...
# вычитание (__sub__()), умножение (__mul__()), деление (__floordiv__, __truediv__()). Эти методы должны применяться
...
class Cell:
    ...
    def make_order(self, row_len):
        result_list = ['*' * row_len] * (self.amount // row_len)
        if self.amount % row_len:
            result_list.append('*' * (self.amount % row_len))
        return '\n'.join(result_list)
